...
在小增益定理的加持下，对于未知的 $\Delta$，我们可以将其看作最不利包络（加权函数）$W$ 与形式未知但 {{< math >}}$\mathcal{H}_\infty${{< /math >}} 范数小于 $1$ 的 $\delta$ 的组合，即 $\Delta = W \delta$。于是 $M$-$\Delta$ 系统可以分解为 $MW$-$\delta$，环路稳定性可以根据 {{< math >}}$\left\lVert MW \right\rVert_\infty${{< /math >}} 判断。因此，不确定性建模的核心在于其传递函数矩阵包络的确定。
