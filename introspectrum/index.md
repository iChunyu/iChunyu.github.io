...
{{< image src="./FourierSeriesShow.png" caption="周期信号的正弦分解示意图" width="70%" >}}


提到分解，不免会想到矢量的分解。对于一个给定的矢量，如果想将它分解为不同矢量的叠加，通常需要选择一组正交的单位基矢（坐标系），然后通过内积计算向量在不同基矢上的投影（坐标），最后就可以通过投影和基矢来表达原来的矢量。


{{< image src="./vecProj.png" caption="矢量的分解" width="35%" >}}


于是，我们可以类比地选择 $\cos n \omega_0 t$ 和 $\sin n \omega_0 t$ 为函数空间的基；矢量内积的相乘、相加运算类比过来就是相乘、积分，而由于是周期信号，因此积分区间只取一个周期即可；最后只要按照内积的定义计算相应的“坐标”即可知道各个正弦信号对应的幅值。这就是函数的 [正交投影]({{< ref "../../math/orthogonalProjection/index.md" >}})。
...
{{< image src="./FourierSeries.png" caption="方波的傅里叶级数示意图" width="70%" >}}
...
{{< image src="./FourierTransform.png" caption="非周期信号的傅里叶变换示意图" width="70%" >}}
...
{{< image src="./sample.png" caption="时域信号的采样" width="50%" >}}
...
{{< image src="./aliasing.png" caption="频率混叠示意图" width="70%" >}}
...
{{< image src="./window.png" caption="实际信号的有限窗长示意图" width="50%" >}}
...
{{< image src="./window-function.png" caption="频率泄露示意图" width="40%" >}}
...
{{< image src="./freqDiscrete.png" caption="频域采样与栅栏效应示意图" width="70%" >}}
...
{{< image src="./freqs-resolution.png" caption="频率分辨率示意图" width="70%" >}}
...
此外，通常的频域采样是为了确保采样后的数据能够根据逆变换公式反演出时域数据，如果只关注频域特性，对频域采样不会具有太高的要求。例如 [LPSD 功率谱估计]({{< ref "../../signal/lpsd/index.md" >}}) 就采用了非均匀的频域采样。
